...
{
    if ([[NSBundle mainBundle] pathForResource:@"MobiPubSDK" ofType:@"bundle"] != nil) {
        return [@"MobiPubSDK.bundle" stringByAppendingPathComponent:resourceName];
    }
    else {
        ...
        NSBundle *resourceBundle = [NSBundle bundleForClass:NSClassFromString(@"MobipubSDK")];
...
{
    switch (type) {
        case MobiInterstitialOrientationTypePortrait: return UIInterfaceOrientationMaskPortrait;
        case MobiInterstitialOrientationTypeLandscape: return UIInterfaceOrientationMaskLandscape;
