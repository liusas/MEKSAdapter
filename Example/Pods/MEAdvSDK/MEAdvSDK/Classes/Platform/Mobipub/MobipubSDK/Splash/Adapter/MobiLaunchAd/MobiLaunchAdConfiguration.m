...
//MARK: 公共
@implementation MobiLaunchAdConfiguration

@end

//MARK: 图片广告相关
@implementation MobiLaunchImageAdConfiguration

+ (MobiLaunchImageAdConfiguration *)defaultConfiguration{
    //配置广告数据
    MobiLaunchImageAdConfiguration *configuration = [MobiLaunchImageAdConfiguration new];
    //广告停留时间
    ...
    //缓存机制
    configuration.imageOption = MobiLaunchAdImageDefault;
    //图片填充模式
    configuration.contentMode = UIViewContentModeScaleToFill;
    //广告显示完成动画
    configuration.showFinishAnimate = MobiShowFinishAnimateFadein;
     //显示完成动画时间
    configuration.showFinishAnimateTime = showFinishAnimateTimeDefault;
    //跳过按钮类型
    configuration.skipButtonType = MobiSkipTypeTimeText;
...
#pragma mark - 视频广告相关
@implementation MobiVideoAdConfiguration
+(MobiVideoAdConfiguration *)defaultConfiguration{
    //配置广告数据
    MobiVideoAdConfiguration *configuration = [MobiVideoAdConfiguration new];
    //广告停留时间
    configuration.duration = 5;
    //广告frame
    configuration.frame = [UIScreen mainScreen].bounds;
