...
{
    MobiTimer *timer = [[MobiTimer alloc] init];
    timer.target = target;
    ...
    // 只要 timer 到后来能够设置成 invalidate,就可以打破循环引用
...
- (void)scheduleNow
{
    /*
     ...
     `MPConsentManager.sharedManager` -> `newNextUpdateTimer` -> `MobiTimer.scheduleNow` ->
...
//                MPLogDebug(@"Could not schedule invalidated MobiTimer (%p).", self);
            return;
        }

        if (self.isCountdownActive) {
//                MPLogDebug(@"Tried to schedule an MobiTimer (%p) that is already ticking.",self);
            return;
        }
...
- (void)pause
{
    @synchronized (self) {
        if (!self.isCountdownActive) {
//            MPLogDebug(@"No-op: tried to pause an MobiTimer (%p) that was already paused.", self);
            return;
        }

        if (![self.timer isValid]) {
//            MPLogDebug(@"Cannot pause invalidated MobiTimer (%p).", self);
            return;
        }
        ...
        if (secondsLeft <= 0) {
//            MPLogInfo(@"An MobiTimer was somehow paused after it was supposed to fire.");
        } else {
//            MPLogDebug(@"Paused MobiTimer (%p) %.1f seconds left before firing.", self, secondsLeft);
        }
...
//            MPLogDebug(@"%s `selector` is unexpectedly nil. Return early to avoid crash.", __FUNCTION__);
        ...
        // performSelector使用时会让self的引用计数加1,执行完再减1
