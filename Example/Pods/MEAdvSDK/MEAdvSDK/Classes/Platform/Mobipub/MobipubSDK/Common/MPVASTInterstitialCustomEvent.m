...
- (void)showInterstitialFromRootViewController:(UIViewController *)rootViewController {
    if (self.playerViewController.presentingViewController) {
        ...
        [rootViewController presentViewController:self.playerViewController animated:Mobi_ANIMATED completion:^{
...
- (void)loadAd {
    void (^fetchAdCompletion)(NSError *) = ^(NSError *fetchAdError) {
        ...
        } else if ([self.delegate conformsToProtocol:@protocol(MobiRewardedVideoCustomEventDelegate)]) {
...
/**
 ...
 Note: `MobiConfig.precacheRequired` is ignored because video precaching is always enforced
 ...
 */
- (void)fetchAndLoadAdWithConfiguration:(MobiConfig *)configuration fetchAdCompletion:(void(^)(NSError *))fetchAdCompletion {
...
- (void)dismissPlayerViewController {
    [self.playerViewController dismissViewControllerAnimated:Mobi_ANIMATED completion:nil];
...
@implementation MPVASTInterstitialCustomEvent (MobiRewardedVideoCustomEvent)
...
- (void)videoPlayerContainerViewDidCompleteVideo:(MPVideoPlayerContainerView *)videoPlayerContainerView duration:(NSTimeInterval)duration {
    ...
    if ([self.delegate conformsToProtocol:@protocol(MobiPrivateRewardedVideoCustomEventDelegate)]) {
#warning 此处服务端支持 reward 奖励时再处理
//        [self.delegate rewardedVideoShouldRewardUserForCustomEvent:self reward:self.delegate.configuration.selectedReward];
        [self.delegate rewardedVideoShouldRewardUserForCustomEvent:self reward:0];
    }
}
...
- (void)videoPlayerContainerView:(MPVideoPlayerContainerView *)videoPlayerContainerView
    ...
    switch (event) {
        case MPVideoPlayerEvent_ClickThrough: {
            ...
            if (self.adConfig.clickTrackingURLs.count != 0) { // prevent crash: nil element in set
                [self.vastTracking uniquelySendURLs:self.adConfig.clickTrackingURLs];
            }
...
- (void)videoPlayerContainerView:(MPVideoPlayerContainerView *)videoPlayerContainerView
    ...
    if (self.adConfig.clickTrackingURLs.count != 0) {
        [urls addObjectsFromArray:self.adConfig.clickTrackingURLs];
    }
