...
static NSString * const kResolverErrorDomain = @"com.mobipub.resolver";
...
- (void)start
{
    ...
    if (info) {
        ...
        MobiURLRequest *request = [[MobiURLRequest alloc] initWithURL:self.originalURL];
...
- (MPURLActionInfo *)actionInfoFromURL:(NSURL *)URL error:(NSError **)error;
{
    ...
    if (storeKitParameters != nil) {
        ...
        // 海外的 url 有一定的标准,primaryUrl 等字段都在 url 里,如下所示
// deeplink+://navigate?primaryUrl=twitter%3A%2F%2Ftimeline&primaryTrackingUrl=https%3A%2F%2Fmopub.com%2Fclicktracking&primaryTrackingUrl=https%3A%2F%2Fmopub.com%2Fmopubtracking&fallbackUrl=https%3A%2F%2Fmobile.twitter.com
