...
+ (void)parseVASTResponseFromData:(NSData *)data depth:(NSInteger)depth completion:(void (^)(MPVASTResponse *response, NSError *error))completion
{
    ...
    for (MPVASTWrapper *wrapper in wrappers) {
        [MobiHTTPNetworkSession startTaskWithHttpRequest:[MobiURLRequest requestWithURL:wrapper.VASTAdTagURI] responseHandler:^(NSData * _Nonnull data, NSHTTPURLResponse * _Nonnull response) {
