...
@property (nonatomic, strong) id<MobiAnalyticsTracker> analyticsTracker; // Note: only send URL's with `uniquelySendURLs`
...
- (instancetype)initWithVideoConfig:(MPVideoConfig *)videoConfig videoURL:(NSURL *)videoURL {
    ...
    if (self) {
        ...
        _videoURL = videoURL;
        _analyticsTracker = [MobiAnalyticsTracker sharedTracker];
