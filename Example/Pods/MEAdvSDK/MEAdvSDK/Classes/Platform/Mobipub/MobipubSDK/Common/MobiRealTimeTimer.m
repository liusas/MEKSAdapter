...
@interface MobiRealTimeTimer ()

@property (strong, nonatomic) MobiTimer *timer;
@property (copy, nonatomic) void (^block)(MobiRealTimeTimer *);
...
@implementation MobiRealTimeTimer

- (instancetype)initWithInterval:(NSTimeInterval)interval
                           block:(void(^)(MobiRealTimeTimer *))block {
    if (self = [super init]) {
        _interval = interval;
        _block = block;
    }
...
- (void)setTimerWithCurrentTimeInterval {
    self.timer = [MobiTimer timerWithTimeInterval:self.currentTimeInterval
                                         target:self
