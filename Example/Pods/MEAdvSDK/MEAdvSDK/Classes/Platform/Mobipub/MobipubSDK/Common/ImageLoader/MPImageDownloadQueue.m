        ...
        [self.imageDownloadQueue addOperationWithBlock:^{
            @autoreleasepool {
                if ([[MPNativeCache sharedCache] cachedDataExistsForKey:imageURL.absoluteString] && useCachedImage) {
                    ...
                    [MobiHTTPNetworkSession startTaskWithHttpRequest:[MobiURLRequest requestWithURL:imageURL] responseHandler:^(NSData * _Nonnull responseData, NSHTTPURLResponse * _Nonnull response) {
                        data = responseData;
