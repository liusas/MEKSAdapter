...
#import "MobiXFLoaderURLConnection.h"
#import "MobiXFVideoRequestTask.h"
#import "MobiXCHudHelper.h"

#define kScreenHeight ([UIScreen mainScreen].bounds.size.height)
#define kScreenWidth ([UIScreen mainScreen].bounds.size.width)
#define IOS_VERSION  ([[[UIDevice currentDevice] systemVersion] floatValue])
NSString *const kXFPlayerStateChangedNotification    = @"XFPlayerStateChangedNotification";
NSString *const kXFPlayerProgressChangedNotification = @"XFPlayerProgressChangedNotification";
NSString *const kXFPlayerLoadProgressChangedNotification = @"XFPlayerLoadProgressChangedNotification";


@interface MobiXFPlayer ()<MobiXFloaderURLConnectionDelegate>

@property (nonatomic        ) XFPlayerState  state;
...
@property (nonatomic, strong) MobiXFLoaderURLConnection *resouerLoader;
...
- (void)setIsMuteds:(BOOL)isMuteds{
    _isMuteds = isMuteds;
    if (isMuteds) {
...
- (void)setContentURL:(NSURL *)contentURL {
    ...
    self.videoAsset  = [AVURLAsset URLAssetWithURL:contentURL options:nil];
...
- (void)playWithUrl:(NSURL *)url showView:(UIView *)showView showType:(NSInteger)type
{
    ...
    if (![str hasPrefix:@"http"]) {
        ...
        self.resouerLoader          = [[MobiXFLoaderURLConnection alloc] init];
    ...
    if ([url.scheme isEqualToString:@"file"]) {
        ...
        if (self.state == TBPlayerStatePlaying) {
            [[NSNotificationCenter defaultCenter] postNotificationName:kXFPlayerStateChangedNotification object:nil];
        } else {
    ...
    //[[MobiXCHudHelper sharedInstance] showHudAcitivityOnWindow];
    
    [[NSNotificationCenter defaultCenter] postNotificationName:kXFPlayerProgressChangedNotification object:nil];
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context
{
    ...
    if ([keyPath isEqualToString:@"status"]) {
        if ([playerItem status] == AVPlayerStatusReadyToPlay) {
            ...
            [[NSNotificationCenter defaultCenter] postNotificationName:@"statusToPlay" object:nil userInfo:@{@"duration":[NSString stringWithFormat:@"%.f",duration]}];
...
- (void)monitoringPlayback:(AVPlayerItem *)playerItem
{
    ...
    self.playbackTimeObserver = [self.player addPeriodicTimeObserverForInterval:CMTimeMake(1, 1) queue:NULL usingBlock:^(CMTime time) {
        ...
        if (strongSelf.delegate && [strongSelf.delegate respondsToSelector:@selector(playWithCurrent:totalDuration:playProgress:)]) {
            [strongSelf.delegate playWithCurrent:strongSelf.current totalDuration:strongSelf.duration playProgress:strongSelf.progress];
        }
...
- (void)setLoadedProgress:(CGFloat)loadedProgress
{
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:kXFPlayerLoadProgressChangedNotification object:nil];
}

- (void)setState:(XFPlayerState)state
{
    if (state != TBPlayerStateBuffering) {
        //[[MobiXCHudHelper sharedInstance] hideHud];
    }
...
{
    _navBar.backgroundColor = [self colorWithHex:0x000000 alpha:0];
    ...
    if (!self.totolTimeLabel) {
        ...
        self.totolTimeLabel.hidden = YES;
