...
@property (nonatomic, strong) MobiTimer *adPropertyUpdateTimer;
...
- (instancetype)initWithAdViewFrame:(CGRect)adViewFrame
              supportedOrientations:(MobiInterstitialOrientationType)orientationType
                           ...
                           delegate:(id<MRControllerDelegate>)delegate
{
    if (self = [super init]) {
        ...
        _forceOrientationMask = MobiInterstitialOrientationTypeToUIInterfaceOrientationMask(orientationType);
        ...
        _adPropertyUpdateTimer = [MobiTimer timerWithTimeInterval:kAdPropertyUpdateTimerInterval
                                                         target:self
...
- (void)loadAdWithConfiguration:(MobiConfig *)configuration
{
    if (self.isAdLoading) {
        return;
    }
...
//    self.allowCustomClose = configuration.mraidAllowCustomClose || configuration.isMoVideo;
    ...
    [self.mraidBridge loadHTMLString:HTML
                             baseURL:[NSURL URLWithString:[MobiAPIEndpoints baseURL]]
     ];
}
...
- (void)loadTwoPartCreativeFromURL:(NSURL *)url
{
    ...
    MPURLRequest * request = [MobiURLRequest requestWithURL:url];
...
- (void)bridge:(MRBridge *)bridge handleNativeCommandSetOrientationPropertiesWithForceOrientationMask:(UIInterfaceOrientationMask)forceOrientationMask
{
    ...
    if (![[UIApplication sharedApplication] mobi_supportsOrientationMask:forceOrientationMask]) {
        return;
    }
    ...
    BOOL inSameOrientation = [[UIApplication sharedApplication] mobi_doesOrientation:MPInterfaceOrientation() matchOrientationMask:forceOrientationMask];
